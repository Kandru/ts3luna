...
public class TS3LoginModule {
	...
	public void sendPasswordToUser(int dbID, String password){
		...
		client.onSuccess(dbClientInfo -> {sendPasswordToDBId(dbClientInfo, password);});
		client.onFailure( result -> {log.warn("Failed to get db client info for db id \""+dbID+"\"");});
	}
	
	private void sendPasswordToDBId(DatabaseClientInfo dbClientInfo, String password){
	...
	private void sendPasswordToClient(ClientInfo clientInfo, String password) {
		...
		CommandFuture<Boolean> privateMessageFuture = ts3api.sendPrivateMessage(clientInfo.getId(), password);
