...
public class TS3LoginModule {
     ...
     * Constructor
     *
     * @param ts3apiAsync {@link TS3ApiAsync} asyncApi to communicate with TS3.
    ...
    public void sendPasswordToUser(int dbID, String password) {
        ...
        client.onSuccess(dbClientInfo -> {
            sendPasswordToDBClient(dbClientInfo, password);
        });
        client.onFailure(result -> {
            log.warn("Failed to get db client info for db id \"{}\"", dbID);
        });
    }
    ...
    private void sendPasswordToDBClient(DatabaseClientInfo dbClientInfo, String password) {
        ...
        clientInfoFuture.onSuccess(clientInfo -> {
            sendPasswordToClient(clientInfo, password);
        });
        clientInfoFuture.onFailure(result -> {
            log.warn("Failed to get client info for client uid \"{}\"", dbClientInfo.getUniqueIdentifier());
        });
    }
    ...
    private void sendPasswordToClient(ClientInfo clientInfo, String password) {
        ...
        privateMessageFuture.onFailure(result -> {
            log.warn("Failed to send private message to user with uid \"{}\"", clientInfo.getUniqueIdentifier());
        });
    }

    /**
     * Extracts the authorization roles for the given client info
     *
     * @param clientInfo The client to be analyzed
     * @return A set of roles (without the ROLE_ prefix)
     */
    public Set<String> getRoles(ClientInfo clientInfo) {
        Set<String> result = new HashSet<>();
        if (properties.getAdmins().stream().anyMatch(item -> item.equals(clientInfo.getUniqueIdentifier())))
            result.add("ADMIN");
        return result;
    }
